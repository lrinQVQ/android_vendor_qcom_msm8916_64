...
restorepathFile=/data/data/com.android.backup/files/restore_path

if [ ! -f "$restorepathFile" ]
then
log -p e -t BackUp "The restore path isn't exist use default path to restore"
else
read backpath < "$restorepathFile"
DstPath="$backpath/backup/App"
CfgPath="$backpath/backup/recoverAppData.txt"
rm -f "$restorepathFile"
fi

log -p d -t BackUp "SrcPath is $SrcPath"
