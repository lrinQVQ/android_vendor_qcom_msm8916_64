...
typedef enum
{
  ...
  /* Command to notify CNE about the change in network handle */
  CNE_NOTIFY_NETWORK_REQUEST_INFO_CMD = 37,
  CNE_NOTIFY_WWAN_SUBTYPE = 38,
  CNE_NOTIFTY_NAT_KEEP_ALIVE_RESULT_CMD = 39,
  CNE_NOTIFY_MOBILE_DATA_ENABLED = 40,
  CNE_NOTIFY_QUOTA_INFO_QUERY_RESULT  = 41,
/**
...
typedef enum
{
  ...
  CNE_REQUEST_UPDATE_POLICY = 30,
  CNE_REQUEST_START_NAT_KEEP_ALIVE = 32,
  CNE_REQUEST_STOP_NAT_KEEP_ALIVE = 33,
  CNE_REQUEST_QUOTA_INFO_QUERY = 34
} cne_msg_enum_type;
...
typedef enum
{
  //Keep in sync with CnoNetType enum
...
typedef struct _ratInfo {
    ...
    _ratInfo(): rat(CNE_RAT_NONE), netHdl(0), status(-1) {
...
typedef struct  _WlanInfo{
    ...
    _WlanInfo(): type(-1), status(-1), rssi(-127), netHdl(0), isAndroidValidated(false) {
    ...
    _WlanInfo(const struct _WlanInfo &src):
      ...
      strncpy(ssid, src.ssid, CNE_MAX_SSID_LEN);
      strncpy(bssid, src.bssid, CNE_MAX_BSSID_LEN);
      strncpy(ipAddr, src.ipAddr, CNE_MAX_IPADDR_LEN);
      strncpy(iface, src.iface, CNE_MAX_IFACE_NAME_LEN);
      strncpy(ipAddrV6, src.ipAddrV6, CNE_MAX_IPADDR_LEN);
      strncpy(ifaceV6, src.ifaceV6, CNE_MAX_IFACE_NAME_LEN);
      strncpy(timeStamp, src.timeStamp, CNE_MAX_TIMESTAMP_LEN);
      for (int i = 0; i < CNE_MAX_DNS_ADDRS; i++)
      {
          strncpy(dnsInfo[i], src.dnsInfo[i], CNE_MAX_IPADDR_LEN);
      }
    }
...
typedef struct _WwanInfo  {
    ...
    _WwanInfo(): type(-1), status(-1), rssi(-127),
        roaming(-1), subrat(CNE_NET_SUBTYPE_UNKNOWN), netHdl(0) {
            bzero(ipAddr, CNE_MAX_IPADDR_LEN);
            bzero(iface, CNE_MAX_IFACE_NAME_LEN);
            bzero(ipAddrV6, CNE_MAX_IPADDR_LEN);
            bzero(ifaceV6, CNE_MAX_IFACE_NAME_LEN);
...
typedef struct CneWlanFamType
{
  cne_fam_type family;
  bool isAndroidValidated;
  CneWlanFamType(): family(CNE_FAM_NONE), isAndroidValidated(false) {}
};
