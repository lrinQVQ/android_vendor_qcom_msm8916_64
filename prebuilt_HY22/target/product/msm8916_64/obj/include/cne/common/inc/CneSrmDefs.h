...
typedef struct _Wlan {
  ...
  _Wlan():type(-1), status(-1), softApStatus(CNE_SOFTAP_STATE_UNKNOWN),
    rssi(-127), netHdl(0), isAndroidValidated(false) {
...
enum SrmEvent {
  ...
  SRM_QUOTA_REACHED_QUERY_RESULT
};
...
struct CneSrmNetworkStatusEventData {
  ...
  CneSrmNetworkStatusEventData():
      softApStatus(CNE_SOFTAP_STATE_UNKNOWN),
      status(CNE_NETWORK_UNKNOWN),
      ssid(NULL),
      bssid(NULL),
      iface(NULL),
      mccMnc(NULL),
